...
class aircraft
{
	...
	public:
		...
		nav_msgs::Odometry get_current_pose_enu()
		{
			return current_pose;
		}

		//call back functions
		void state_cb(mavros_msgs::State::ConstPtr& msg)
		{
		  current_state = *msg;
		}
		void pose_cb(nav_msgs::Odometry::ConstPtr& msg)
		{
		  current_pose = *msg;
		...
		{
		  heading = heading + correction_heading + local_offset;
		...
		void set_destination(float x, float y, float z, float psi)
		{
			...
			float Xlocal = x*cos((correction_heading + local_offset - 90)*deg2rad) - y*sin((correction_heading + local_offset - 90)*deg2rad);
			float Ylocal = x*sin((correction_heading + local_offset - 90)*deg2rad) + y*cos((correction_heading + local_offset - 90)*deg2rad);
			float Zlocal = z;
		...
		void init_publisher_subscriber(ros::NodeHandle controlnode)
		{
			...
			// currentPos = controlnode.subscribe<nav_msgs::Odometry>("mavros/global_position/local", 10, this->pose_cb);
			// state_sub = controlnode.subscribe<mavros_msgs::State>("mavros/state", 10, this->state_cb);
